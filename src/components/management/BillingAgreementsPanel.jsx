...
import { Building2, CalendarRange, CheckCircle2, Loader2, Pencil, Plus, ReceiptText, Trash2, UserPlus, WalletCards } from 'lucide-react';
import EditAgreementDialog from '@/components/management/EditAgreementDialog';
...
export default function BillingAgreementsPanel({ agreements = [], statements = [], customers = [], orders = [], selectedUnitId, defaultUnitId, onRefresh }) {
  ...
  const run = async (name, payload, success) => {
    ...
    try {
      ...
      const messages = {
        ...
        agreement_has_statements: 'Não é possível excluir: o convênio já possui fechamentos.',
        agreement_has_orders: 'Não é possível excluir: existem pedidos vinculados a este convênio.',
        agreement_code_already_exists: 'Já existe um convênio com este código.',
  ...
  return (
    <section className="space-y-5">
      ...
      <EditAgreementDialog agreement={editingAgreement} customers={customers} onOpenChange={setEditingAgreement} onSaved={() => { setEditingAgreement(null); onRefresh?.(); }} />
...
function AgreementAssignments({ agreement, customers, orders, busy, run }) {
  ...
  return <div className="mt-4 grid gap-3 border-t border-white/10 pt-4 sm:grid-cols-2"><div className="space-y-2"><p className="text-xs font-medium text-white/45">Vincular cliente</p><div className="flex gap-2"><select value={customerId} onChange={(event) => setCustomerId(event.target.value)} className="h-9 min-w-0 flex-1 rounded-md border border-white/10 bg-black/25 px-2 text-xs text-white"><option value="">Selecione</option>{customers.filter((customer) => customer.unit_id === agreement.unit_id && !(agreement.customer_ids || []).includes(customer.id)).slice(0, 500).map((customer) => <option key={customer.id} value={customer.id}>{customer.full_name}</option>)}</select><Button size="sm" variant="outline" disabled={!customerId || busy} onClick={async () => { await run('manage_billing_agreement', { action: 'assign_customer', billing_agreement_id: agreement.id, customer_id: customerId }, 'Cliente vinculado.'); setCustomerId(''); }} className="shrink-0 border-white/10 bg-white/5 text-xs"><UserPlus className="mr-1.5 h-3.5 w-3.5" />Vincular</Button></div></div><div className="space-y-2"><p className="text-xs font-medium text-white/45">Autorizar pedido faturado</p><div className="flex gap-2"><select value={orderId} onChange={(event) => setOrderId(event.target.value)} className="h-9 min-w-0 flex-1 rounded-md border border-white/10 bg-black/25 px-2 text-xs text-white"><option value="">Selecione</option>{orders.slice(0, 300).map((order) => <option key={order.id} value={order.id}>{order.ticket_number || order.id} · {money(Number(order.total_amount || 0) - Number(order.paid_amount || 0))}</option>)}</select><Button size="sm" variant="outline" disabled={!orderId || busy} onClick={async () => { await run('manage_billing_agreement', { action: 'assign_order', billing_agreement_id: agreement.id, order_id: orderId }, 'Pedido autorizado para faturamento.'); setOrderId(''); }} className="shrink-0 border-white/10 bg-white/5 text-xs"><WalletCards className="mr-1.5 h-3.5 w-3.5" />Autorizar</Button></div></div></div>;
}
