...
Deno.serve(async (req) => {
    try {
        ...
        if (isGroupTarget) {
            console.warn("Blocked: attempt to send to a WhatsApp GROUP id (Moinhos):", phone);
        ...
        if (type === 'IMAGE') {
            ...
            payload = { phone, message, optionList, delayMessage: 2 };
        }

        let zapiResponseData = { status: "simulated", messageId: "sim-" + Date.now() };

        if (INSTANCE_ID && TOKEN) {
            console.log(`Sending to Z-API (Moinhos): ${zapiUrl}`, JSON.stringify(payload));
            ...
            console.warn("Z-API Moinhos secrets not configured. Simulating send.");
        }
        ...
        if (!finalConversationId && phone) {
            ...
            if (customers.length > 0) {
                ...
                // Só considera conversas da conexão Moinhos (chats separados por número).
        ...
        console.error("Error in zapi_moinhos_sender:", error);
