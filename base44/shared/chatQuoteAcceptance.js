...
export async function acceptChatQuote({ base44, quote, conversation, currentState, latestText, activePickups = [] }) {
  ...
  if (piecesTotal <= 150 && !choice) {
    Object.assign(currentState, { active_quote_id: quote.id, flow: 'AWAITING_FULFILLMENT_CHOICE' });
  ...
  Object.assign(currentState, { active_quote_id: quote.id, flow: 'AWAITING_PAYMENT_METHOD', fulfillment_choice: choice, delivery_requested: choice === 'pickup', temp_items: [] });
  await base44.asServiceRole.entities.Conversation.update(conversation.id, { metadata: { ...currentState } });
